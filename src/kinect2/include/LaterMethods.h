...
//    static void RenderBBox2DWithScore(const Vector<double> &bbox, QImage& image, int r, int g, int b, int lineWidth);
    ...
    static void conv1D(unsigned char *vec, const vector<double>& kernel, bool flag);
    static void MorphologyErode(unsigned char *vec);
    static void MorphologyDilate(unsigned char *vec);
